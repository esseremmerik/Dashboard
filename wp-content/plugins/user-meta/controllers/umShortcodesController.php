...
class umShortcodesController {
    ...
    function init( $atts ){     
        ...
        if( in_array( $actionType, array( 'registration', 'profile', 'both', 'none' ) ) )
            return $userMeta->userUpdateRegisterProcess( $actionType, $form ); 
        ...
        $output .= $userMeta->userUpdateRegisterProcess( $type, $form );   
        return $output;*/
    }
    
    
    function backwordInit(){
        return $userMeta->userUpdateRegisterProcess( 'profile', 'profile' );
    }
    
    /**
     * Sleep
     * 
     * Replace with: umMethods(models)::userUpdateRegisterProcess()
     * 
     */
