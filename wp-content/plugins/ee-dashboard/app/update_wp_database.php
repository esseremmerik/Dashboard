...
require BASE_PATH . 'config/solve360_cred.php';

function update_timerecords(){  
	global $wpdb;
	$startDate = '2013-02-01';
	$endDate = '2013-03-01';
	//$solve360Service = getSolve360Service();
	...
	foreach ($result->timerecords->timerecord as $timerecord)
	{
		...
		$result = $wpdb->insert($wpdb->prefix. 'solve360_timerecord',$data);
	} 
	//var_dump($wpdb->get_results( "SELECT * FROM wp_solve360_timerecord" ));
}
function update_tasks(){  
	...
	$type 			= 14;
	$solve360Service= new Solve360Service(USER, TOKEN);
	$result 		= $solve360Service->getActivities($type, $last, $startDate, $endDate);
	
	foreach ($result->activities->activity as $task)
	{
		$data = array(
...
function update_ownership(){
	global $wpdb;
	$solve360Service= new Solve360Service(USER, TOKEN);
	$result 		= $solve360Service->getOwnership();
	...
	//var_dump($wpdb->get_results( "SELECT * FROM wp_solve360_ownership" ));
}

function update_tasklists(){
	global $wpdb;
	$solve360Service = new Solve360Service(USER, TOKEN);
	$type = 22;
	$last = 'created';
	$start = '2009-01-01';
	$end = '2013-12-20';
	$result = $solve360Service->getActivities($type, $last, $start, $end, $filter = "");
